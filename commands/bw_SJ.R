...
x <- readRDS("/workspaces/seurat_blog/test_data/inputs_fitBeta_one_group.rds")

# commands/r-source-4.5.2/src/library/stats/R/bandwidths.R:42
...
bw_pair_cnts <- function(x, nb, binned)
{
    ...
    if(binned) {
        ...
        xx <- xx - min(xx) + 1
        xxx <- tabulate(xx, nb)
        list(d, .Call(stats_dll$bw_den_binned, xxx))
    } else {
        .Call(stats_dll$bw_den, nb, x)
    }
}

# commands/r-source-4.5.2/src/library/stats/R/bandwidths.R:55
                  ...
                  method = c("ste", "dpi"), tol = 0.1*lower)
{
    ...
    SDh <- function(h) .Call(stats_dll$bw_phi4, n, d, cnt, h)
    TDh <- function(h) .Call(stats_dll$bw_phi6, n, d, cnt, h)
